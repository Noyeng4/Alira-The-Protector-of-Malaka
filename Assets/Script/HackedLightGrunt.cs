    ...
    private void OnEnable()
{
    ...
    private void DetectEnemyOrFollowPlayer()
    {
        ...
        GameObject[] heavyBosses = GameObject.FindGameObjectsWithTag("HeavyBoss");
        ...
        foreach (GameObject heavyBoss in heavyBosses)
        {
            float distanceToHeavyBoss = Vector3.Distance(transform.position, heavyBoss.transform.position);
            if (distanceToHeavyBoss < nearestDistance && distanceToHeavyBoss <= enemyDetectRange)
            {
                nearestDistance = distanceToHeavyBoss;
                currentTarget = heavyBoss.transform;
            }
        }
    ...
    private void AttackTarget()
    {
        if (Time.time >= lastAttackTime + attackCooldown)
        {
            ...
            if (currentTarget != null)
            {
                ...
                else if (currentTarget.CompareTag("HeavyBoss"))
                {
                    HeavyBoss heavyBossScript = currentTarget.GetComponent<HeavyBoss>();
                    if (heavyBossScript != null)
                    {
                        heavyBossScript.TakeDamage(damage);
                    }
                }
            }
