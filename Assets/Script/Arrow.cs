...
public class Arrow : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        else if (other.CompareTag("HeavyBoss")) // Check for MiddleBot tag
        {
            HeavyBoss HeavyBoss = other.GetComponent<HeavyBoss>(); // Get MiddleBot component
            if (HeavyBoss != null)
            {
                HandleHitHeavyBoss(HeavyBoss); // Handle hit logic for MiddleBot
            }
        }
    ...
    private void HandleHitHeavyBoss(HeavyBoss heavyBoss)
    {
        if (isSpecialArrow)
        {
            return;
        ...
        else
        {
            // Calculate damage based on charge level for regular arrows
            int damageToDeal = CalculateDamage();
            heavyBoss.TakeDamage(damageToDeal); // Call method to reduce health
        }
    }
